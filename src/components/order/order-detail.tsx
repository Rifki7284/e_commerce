...
const OrderDetail = ({ order }: Props) => {
  ...
  const downloadAllKeys = () => {
    ...
    keysText += `Purchase Date: ${new Date(
      order.createdAt
    ).toLocaleDateString()}\n\n`;
    keysText += `===========================================\n`;
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-7xl">
      ...
      {/* Status Banner */}
      <div
        className={`bg-linear-to-r ${statusInfo.bgGradient} backdrop-blur-sm border ${statusInfo.borderColor} rounded-2xl p-6 mb-8 relative overflow-hidden`}
      >
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div className="flex-1">
            ...
            <p
              className={`text-2xl sm:text-3xl font-black ${statusInfo.color}`}
            >
              {statusInfo.label}
            </p>
            {isPaidOrCompleted && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {order.orderItems?.map((item) => (
            <div
              key={item.id}
              className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden"
            >
              {/* Product Header */}
                ...
                {isPaidOrCompleted && item.keys && item.keys.length > 0 && (
                  <div className="space-y-3">
                    ...
                    {item.keys.map((key, index) => (
                      <div
                        key={key.id}
                        className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4 hover:bg-slate-900/60 transition-all"
                      >
                        ...
                        <p className="text-xs text-slate-500 mt-2">
                          Delivered:{" "}
                          {new Date(key.gameKey.soldAt).toLocaleDateString(
                            "en-US",
                            {
                              month: "short",
                              day: "numeric",
