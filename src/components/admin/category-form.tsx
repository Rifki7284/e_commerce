...
const CategorySchema = z.object({
  ...
  iconEmoji: z.string().min(1, "Please select an icon"),
});
...
export default function CategoryFormModal({
  ...
  const handleIconSelect = (emoji: string) => {
    ...
    setValue("iconEmoji", emoji, { shouldValidate: true });
  };
  ...
  const onSubmit = async (data: CategoryFormData) => {
    ...
    try {
      ...
      formData.append("iconName", data.iconEmoji);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl p-0 gap-0 max-h-[95vh] flex flex-col">
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto max-h-[60dvh] px-6 py-5 space-y-5">
          {errorMessage && (
