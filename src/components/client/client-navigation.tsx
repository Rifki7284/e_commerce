"use client"
import { ChevronDown, Gamepad2, Home, LogIn, LogOut, Menu, Package, Search, ShoppingBag, ShoppingCart, User, UserPlus, X, Zap } from "lucide-react";
...
const ClientNavigation = ({cartCount, session}: Props) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-slate-700 bg-slate-900/95 backdrop-blur-md shadow-lg shadow-black/20">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <Link href={isAuthenticated ? "/home" : "/login"} className="flex items-center gap-2 group">
            ...
            <div className="flex items-center gap-2">
              ...
              {isAuthenticated ? (
                ...
                <>
                  {/* Desktop Auth Button - For non-authenticated users */}
                  <div className="hidden md:flex items-center gap-2">
                    <button
                      onClick={() => handleNavigation("/login")}
                      className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all duration-200 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 font-medium"
                    >
                      <User size={18} />
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      setSearchQuery(item);
                      router.push(
                        `/product?search=${encodeURIComponent(item)}`
                      );
                      setSearchDrawerOpen(false);
                    }}
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      router.push(
                        `/product?search=${encodeURIComponent(item.name)}`
                      );
                      setSearchDrawerOpen(false);
                    }}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isAuthenticated ? (
              <div className="space-y-4">
                ...
                <div className="px-4 py-3 bg-slate-800/50 rounded-lg border border-slate-700">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-12 h-12 bg-linear-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30 rounded-full flex items-center justify-center">
              ...
              <div className="space-y-4">
                ...
                <div className="px-4 py-6 bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-700/50 text-center">
                  <div className="w-16 h-16 bg-blue-600 rounded-xl flex items-center justify-center text-white mx-auto mb-3 shadow-lg shadow-blue-500/20">
                    <Gamepad2 size={32} />
                  </div>
                  <h3 className="text-lg font-bold text-white mb-2">Welcome to GameKeys!</h3>
                  <p className="text-sm text-slate-400">Sign in to access your library and exclusive deals</p>
                </div>

                {/* Auth Button */}
                <button
                  onClick={() => handleNavigation("/login")}
                  className="w-full flex items-center justify-center gap-3 px-4 py-3.5 text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-all duration-200 font-medium shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30"
                >
                  <User size={20} />
                  <span>Login / Register</span>
                </button>
      ...
      {isAuthenticated && cartOpen && <ShoppingCartModal onClose={() => setCartOpen(false)} />}
    </>
  );
};
