...
export default function ShoppingCart({ onClose }: ShoppingCartProps) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.log(
        data.reuse ? "🟡 Reusing existing order" : "🆕 New order created"
      );
  ...
  if (loading) {
    return (
      <>
        ...
        />
        <div
          className={`fixed inset-y-0 right-0 z-50 w-full max-w-md bg-linear-to-br from-slate-900 to-slate-950 shadow-2xl transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 right-0 z-50 w-full max-w-md bg-linear-to-br from-slate-900 to-slate-950 shadow-2xl flex flex-col border-l border-slate-700 transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isClosing
            ? "translate-x-full"
            : isOpen
            ? "translate-x-0"
            ...
            <div className="space-y-4">
              {cart.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`bg-slate-800/30 rounded-xl p-4 border border-slate-700 transition-all duration-200 ${
                      isRemoving
                        ? "opacity-40 scale-95"
                        : "opacity-100 hover:border-blue-500/30"
                    }`}
                  >
