...
import CategoryFormModal from "@/components/admin/category-form"
interface Category {
  id: number
  name: string
  slug: string
...
export default function AdminPage() {
  ...
  const getCategory = async () => {
    setLoading(true)
    const res = await fetch(`/api/category?page=${currentPage}&perPage=${perPage}&search=${search}`)
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">Add, edit, or remove category </p>
          </div>
          <Button
            onClick={() => setIsModalOpen(true)}
            className="inline-flex items-center gap-2 bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 dark:from-blue-500 dark:to-purple-500 dark:hover:from-blue-600 dark:hover:to-purple-600 text-white font-semibold shadow-md hover:shadow-lg transition-all"
          >
